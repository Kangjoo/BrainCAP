...
step_dict = {'concatenate_bolds':f'{script_path}/braincap_concatenate.py',
             'prep':f'{script_path}/braincap_prep.py',
             'clustering':f'{script_path}/braincap_clustering.py',
             'post':f'{script_path}/braincap_post.py',
             'temporal_metrics':f'{script_path}/braincap_post.py'}
...
parser.add_argument("--config", type=file_path, required=True, help="Path to config file with BrainCAP parameters")
parser.add_argument("--steps", type=str, help="Comma seperated list of BrainCAP steps to run. Can also be specified in config")
...
for step in args['steps']:
        ...
        for ckey, cval in step_args['cluster_args'].items():
            if isinstance(cval, list):
                if not var_key: 
                    ...
                    raise pe.StepError(step="BrainCAP Clustering Orchestration",
    ...
    for job in range(jobs):
        ...
        if jobs == 1:
            step_args['log_path'] = os.path.join(step_args['logs_folder'], f'BrainCAP_{step}_{timestamp}.log')
            if sched_type != "NONE":
                sched_log = os.path.join(step_args['logs_folder'], f'BrainCAP_{sched_type}_{step}_{timestamp}.log')
                logs.append(sched_log)
                job_name = f"{step}"
        else:
            step_args['log_path'] = os.path.join(step_args['logs_folder'], f'BrainCAP_{step}_perm{split}_{timestamp}.log')
            if sched_type != "NONE":
                sched_log = os.path.join(step_args['logs_folder'], f'BrainCAP_{sched_type}_{step}_perm{split}_{timestamp}.log')
                logs.append(sched_log)
        ...
        if ntasks != 1 and sched_type.upper() != "NONE":
            task = "${PERM}"
            step_args['log_path'] = os.path.join(step_args['logs_folder'], f'BrainCAP_{step}_perm{split}_{task}_{timestamp}.log')
        ...
        for arg in arg_dict['required'][step]:
            if arg not in step_args.keys():
                print(f"ERROR! Missing required argument '{arg}' for BrainCAP '{step}'. Exiting...")
                exit()
